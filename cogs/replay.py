...
class MafiaReplay(commands.Cog):
    ...
    async def get_list(self, ctx):
        results = await (await (await aiosqlite.connect("db/db.sqlite")).execute(f'SELECT * FROM mafia ORDER BY code')).fetchall()
        if not results:
        ...
        embed = discord.Embed(title="게임 목록", color=0x5865F2, description=content + '```')
        embed.set_footer(text=self.bot.user.name, icon_url=self.bot.user.avatar.url)
        return await ctx.reply(embed=embed)

    @commands.command(aliases=['마피아다시보기'])
    async def replay(self, ctx, code: str = None):
        ...
        result = await (await (await aiosqlite.connect("db/db.sqlite")).execute(f'SELECT * FROM mafia WHERE code = "{code}"')).fetchone()
        if not result:
            embed = discord.Embed(title="Mafia", color=0xED4245,
        ...
        embed = discord.Embed(title=f"요약 - `{code}`", color=0x5865F2)
        get_fields(embed)

...
class Giveaway(commands.Cog):
    ...
    async def _giveaway(self, ctx):
        ...
        em.add_field(name="*gstart <time> <winners> <message/prize>",
                     value="Starts a giveaway for the specified amount of time.", inline=False)
    ...
    async def _gstart(self, ctx, timee: str, winners: str, *, message):
        ...
        winners = int(winners)
        con_time = self.convert(timee)
        if con_time == -1:
            raise CommandInvokeError('Time enterd incorrectly must be s|m|h|d')
        elif con_time == -2:
            raise CommandInvokeError('Time should be int')
        end = datetime.datetime.now() + datetime.timedelta(seconds=con_time)
        end_str = f"{end.year}-{end.month}-{end.day} {end.hour}:{end.minute}:{end.second}"
        #end = datetime.datetime.strftime(end, "%Y %b %d  %I:%M %p")
        timestamp = time.mktime(datetime.datetime.strptime(end_str,'%Y-%m-%d %H:%M:%S').timetuple())
        em = discord.Embed(title=f'{message}',
                           description=f"React with :tada: to enter\nEnds: <t:{str(timestamp)[:-2]}:R> (<t:{str(timestamp)[:-2]}>)\nHosted by: {ctx.author.mention}\nWinners: {winners}",

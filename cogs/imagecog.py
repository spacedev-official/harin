...
class ImageCog(commands.Cog):
    ...
    async def get_bg(self):
        async with self.session.get("https://media.discordapp.net/attachments/884407186854404106/904955535391088680/infobg.png") as resp:
            bg_bytes = await resp.read()
        return bg_bytes

    @staticmethod
    def processing(avatar_bytes: bytes,bg_bytes: bytes) -> BytesIO:
        ...
        with Image.open(BytesIO(avatar_bytes)) as im:
            ...
            with Image.open(BytesIO(bg_bytes)) as background:
                ...
                with Image.new("L", im.size) as mask:
                    ...
                    # we just created.
                    background.paste(rgb_avatar, (740, 135), mask=mask)

...
class MafiaGame(commands.Cog):
    ...
    async def end(self, winner, data, thread, msg):
        conn = await aiosqlite.connect("db/db.sqlite")
        def gen():
        ...
        codes = [c[0] for c in await (await conn.execute('SELECT * FROM mafia')).fetchall()]
        code = gen()
        while code in codes:
            code = gen()

        data['winner'] = winner
        await conn.execute(f'INSERT INTO mafia VALUES ("{code}", "{thread.guild.id}", "{str(data)}")')
        await conn.commit()
